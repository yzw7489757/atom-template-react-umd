...
const { resolve: rootResolve, getVendors} = require('./utils')
const CompressionWebpackPlugin = require('compression-webpack-plugin'); // gzip压缩
const productionGzipExtensions = ['js', 'css'];// 压缩的类型
const AutoDllPlugin = require('autodll-webpack-plugin'); // dll静态资源
const UglifyJsPlugin = require('uglifyjs-3-webpack-plugin'); // 压缩js
...
module.exports = function(webpackEnv) {
  ...
  //用于在传递到alias对象的生产中启用分析的变量。如果传递到生成命令中s，则使用标志
  ...
  return {
    ...
    optimization: {
      ...
      // splitChunks: {
      //   chunks: 'all', // 从哪些chunks里面抽取代码，可选 all，针对异步分割或者全部分割
    ...
    resolve: {
      ...
      alias: {
        '@': rootResolve('./src'),
        public: rootResolve('./public'),
        assets: rootResolve('./src/assets'),
        '@Style': rootResolve('./src/assets/styles'),
        views: rootResolve('./src/views'),
        lib: rootResolve('./src/lib'),
        utils: rootResolve('./src/utils'),
        components: rootResolve('./src/components'),
        // 'react':require.resolve('../../react-code-learn/build/dist/react.development.js'),
        // 'react-dom':require.resolve('../../react-code-learn/build/dist/react-dom.development.js'),
      ...
      plugins: [
        ...
        // new ModuleScopePlugin(paths.appSrc,paths.reactSrc, [paths.appPackageJson]),
      ],
    },
    resolveLoader: {
      plugins: [
