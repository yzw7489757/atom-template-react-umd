...
export default class Highlightable extends Component {
  ...
  shouldComponentUpdate(newProps) {
    return newProps.ranges.length !== this.props.ranges.length || newProps.text !== this.props.text || newProps.enabled !== this.props.enabled;
  }

  getRange(charIndex) {
    return this.props.ranges && this.props.ranges.find(range => charIndex >= range.start && charIndex <= range.end);
  }
  ...
  getLetterNode(charIndex, range) {
    return (
      <Node id={this.props.id} range={range} charIndex={charIndex} key={`${this.props.id}-${charIndex}`} highlightStyle={this.props.highlightStyle}>
  ...
  rangeRenderer(letterGroup, range, textCharIndex, onMouseOverHighlightedWord) {
    return this.props.rangeRenderer ? this.props.rangeRenderer(letterGroup, range, textCharIndex, onMouseOverHighlightedWord) : letterGroup;
  }
  ...
  render() {
    ...
    return (
      <div style={this.props.style} onMouseUp={this.onMouseUp.bind(this)} onDoubleClick={this.onDoubleClick.bind(this)}>
        {newText}
      </div>
    );
  }
